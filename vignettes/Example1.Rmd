...
#Plot Species 1
plot(my.df$RunTime, my.df$Sp1, xlab = "Time Step", ylab = "Number of Adults in Landscape", xlim = c(0,x.limit), ylim = c(0, y.limit), pch = 15, col = 4)
...
x.limit = 15
y.limit = log(7000,10)
pch.vec = c(1, 6,20,0)#One point type for each cell
col.vec = c(3,3,3,3)  #One color for each cell (using the same color to make it clear it is Species 3)

#Plot Cell1
plot(sp.dat.ad3$TimeStep,log(sp.dat.ad3$Cell1,10), xlim = c(0,x.limit), ylim = c(0,y.limit), pch = pch.vec[1], col = col.vec[1], xlab = "Time Step", ylab = "Log10 Number of Adults of Species 3" )

#Plot Cells 2 - 4
for (i in 2:4){
    par(new = T) #Tell R to add next plot to existing plot
    this.cell = sprintf("Cell%s", i)
    plot(sp.dat.ad3$TimeStep, log(sp.dat.ad3[[this.cell]],10), xlim = c(0,x.limit), ylim = c(0, y.limit), pch = pch.vec[i], col = col.vec[i], xlab = "", ylab = "", xaxt = 'n', yaxt = 'n')
    }
...
The first plot shows the patterns described above. (Species 1 = blue squares, Species 2 = red circles, and Species 3 = green triangles). The second plot shows the importance of space. Cell 4 (open squares) serves as a reference - this is the population unaffected by herbicides. The other three Cells all increase, but Cells 1 and 3 (inverted triangles and filled circles) increase faster because they receive more propagules due to their greater proximity to Cell 4. Cell 2 (open circles) increases the slowest.
